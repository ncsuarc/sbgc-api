...
// Conversions for angle in degrees to angle in SBGC 14bit representation, and
// back
...
#define SBGC_DEGREE_TO_ANGLE_INT(val)                                          \
  ((int32_t)(val)*SBGC_ANGLE_FULL_TURN / 360)
#define SBGC_DEGREE_01_TO_ANGLE_INT(val)                                       \
  ((int32_t)(val)*SBGC_ANGLE_FULL_TURN / 3600)
#define SBGC_ANGLE_TO_DEGREE_INT(val)                                          \
  ((int32_t)(val)*360 / SBGC_ANGLE_FULL_TURN)
#define SBGC_ANGLE_TO_DEGREE_01_INT(val)                                       \
  ((int32_t)(val)*3600 / SBGC_ANGLE_FULL_TURN)
...
inline uint8_t SBGC_cmd_control_send(SBGC_cmd_control_t &p,
                                     SBGC_Parser &parser) {
  SerialCommand cmd;
...
inline uint8_t SBGC_cmd_control_ext_send(SBGC_cmd_control_ext_t &p,
                                         SBGC_Parser &parser) {
  SerialCommand cmd;
...
void SBGC_cmd_api_virt_ch_control_pack(SBGC_cmd_api_virt_ch_control_t &p,
                                       SerialCommand &cmd);
inline uint8_t
SBGC_cmd_api_virt_ch_control_send(SBGC_cmd_api_virt_ch_control_t &p,
                                  SBGC_Parser &parser) {
  SerialCommand cmd;
...
inline uint8_t SBGC_cmd_trigger_send(SBGC_cmd_trigger_t &p,
                                     SBGC_Parser &parser) {
  SerialCommand cmd;
...
inline uint8_t SBGC_cmd_servo_out_send(SBGC_cmd_servo_out_t &p,
                                       SBGC_Parser &parser) {
  SerialCommand cmd;
...
void SBGC_cmd_set_adj_vars_pack(SBGC_cmd_set_adj_vars_var_t vars[],
                                uint8_t vars_num, SerialCommand &cmd);
uint8_t SBGC_cmd_set_adj_vars_unpack(SBGC_cmd_set_adj_vars_var_t vars_buf[],
                                     uint8_t &vars_num, SerialCommand &cmd);
inline uint8_t SBGC_cmd_set_adj_vars_send(SBGC_cmd_set_adj_vars_var_t vars[],
                                          uint8_t vars_num,
                                          SBGC_Parser &parser) {
  SerialCommand cmd;
...
typedef struct {
  struct {
    int16_t acc_data;
    int16_t gyro_data;
  } sensor_data[3]; // ACC and Gyro sensor data (with calibration) for current
                    // IMU (see cur_imu field)
  ...
  uint8_t reserved1[4];
  int16_t rc_raw_data[SBGC_RC_NUM_CHANNELS]; // RC signal in 1000..2000 range
                                             // for ROLL, PITCH, YAW, CMD,
                                             // EXT_ROLL, EXT_PITCH channels
  int16_t imu_angle[3]; // ROLL, PITCH, YAW Euler angles of a camera, 16384/360
                        // degrees
  int16_t
      frame_imu_angle[3];  // ROLL, PITCH, YAW Euler angles of a frame, if known
  ...
  uint8_t cur_imu; // actually selecteted IMU for monitoring. 1: main IMU, 2:
                   // frame IMU
  uint8_t cur_profile; // active profile number starting from 0
  uint8_t
      motor_power[3]; // actual motor power for ROLL, PITCH, YAW axis, 0..255
  ...
  uint8_t reserved3;
  int16_t balance_error[3];     // error in balance. Ranges from -512 to 512,  0
  ...
  int8_t imu_temp_celcius; // temperature measured by the main IMU sensor, in
                           // Celsius
  int8_t frame_imu_temp_celcius; // temperature measured by the frame IMU
                                 // sensor, in Celsius
  uint8_t reserved4[38];
} SBGC_cmd_realtime_data_t;

uint8_t SBGC_cmd_realtime_data_unpack(SBGC_cmd_realtime_data_t &p,
                                      SerialCommand &cmd);

inline uint8_t SBGC_cmd_execute_menu_send(uint8_t menu_action,
                                          SBGC_Parser &parser) {
  SerialCommand cmd;
