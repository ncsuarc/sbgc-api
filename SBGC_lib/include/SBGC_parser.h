...
public:
  ...
  static inline void init_checksum(uint8_t &checksum) { checksum = 0; }
...
public:
  ...
  inline uint8_t process_char(uint8_t c) {
    ...
    case STATE_GOT_LEN:
      if (c == (uint8_t)(in_cmd.id + len) &&
          len <= sizeof(in_cmd.data)) { // checksum and size ok
   ...
   */
  uint8_t send_command(uint8_t cmd_id, void *data, uint16_t size,
                       uint8_t wait = 1) {
    if (com_obj != NULL &&
        size <= (SBGC_CMD_MAX_BYTES - SBGC_CMD_NON_PAYLOAD_BYTES)) {
      if (wait ||
          com_obj->getOutEmptySpace() >= size + SBGC_CMD_NON_PAYLOAD_BYTES) {
        com_obj->writeByte(
            SBGC_CMD_START_BYTE);          // protocol-specific start marker
